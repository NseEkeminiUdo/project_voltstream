...
def extract_col_from_json_string(df, json_string: str):
    ...
    df = df.select(
        [
            get_json_object(col(json_string), f"$.{key}").alias(value)
...
def validate_columns(df, **log_info):
    ...
    for field in df.columns:
        ...
        if null_count == 0:
            log(logging.ERROR, f"Pipeline stopped: column '{field}' is all NULL")
            raise RuntimeError(f"Pipeline stopped: column '{field}' is all NULL")
    return df


# This function validates the records in the dataframe and quarantines the
# invalid records
def validate_and_quarantine_rows(df, required_cols, non_negative_cols=None, **log_info):
...
def join_current_and_incoming_data(stg_dim, current_dim, id: str | list):
    if isinstance(id, str):
        id = [id]
    return stg_dim.alias("stg").join(current_dim.alias("current"), on=[*id], how="left")
...
def add_columns_for_idempotency(df, id: list, alias: str, attributes: list):
    return df.filter(col("scd_action").isin(["INSERT", "UPDATE"])).select(
        (xxhash64(*[col(f"{c}") for c in id])).alias(alias),
...
def identify_rows_to_expire(df, id: str):
    return df.filter(col("scd_action") == "UPDATE").select(
        col(id), date_sub(col("valid_from"), 1).alias("valid_to")
    )
...
def update_expired_rows(dim_delta, expire_df, id, **log_info):
    ...
    dim_delta.alias("current").merge(
        expire_df.alias("updates"), f"current.{id} = updates.{id}"
    ).whenMatchedUpdate(
        set={"valid_to": col("updates.valid_to"), "is_current": lit(False)}
    ).execute()
...
def select_station_columns(df):
    ...
    df = df.select(*cols).withColumn(
        "state_or_province",
        when(col("state_or_province") == "NJ", "New Jersey")
        .when(col("state_or_province") == "NY", "New York")
        .otherwise(col("state_or_province")),
    )
    return df
...
def remove_nulls(df):
    return df.filter(col("longitude").isNotNull()).filter(col("latitude").isNotNull())


# This function converts the latitude and longitude columns to double type
def convert_lat_lon_type(df):
    return df.withColumn("latitude", col("latitude").cast(DoubleType())).withColumn(
...
def add_weather_zone_coordinates(df):
    return df.withColumn("weather_zone_lat", round(col("latitude"), 1)).withColumn(
        "weather_zone_lon", round(col("longitude"), 1)
    )
...
def explode_connections(df, **log_info):
    ...
    df = df.select(
        explode_outer(from_json(col("Connections"), explode_conn_schema)).alias("conn"),
        col("station_id"),
...
def transform_weather(df):
    column_exprs = {
        "weather_sk": xxhash64(col("lat"), col("lon"), col("ingest_timestamp")),
        "lat": round(col("lat"), 1),
        "lon": round(col("lon"), 1),
        "weather": col("weather"),
        ...
        "clouds": col("clouds"),
        "dt_utc": from_unixtime(col("dt")).cast(TimestampType()),
    }

    df = (
        df.select(*[expr.alias(name) for name, expr in column_exprs.items()])
        .withColumn("ingest_timestamp", lit(datetime.now()))
