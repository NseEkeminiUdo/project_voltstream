...
def join_tables(df_stations, df_connections, df_weather):
    df = (
        ...
        .withColumn("is_operational", col("s.status_id") == 50)
    )  # indicates if the station is operational or not
    window = Window.partitionBy(col("s.station_id")).orderBy(
        col("s.date_last_status_update").desc()
    )
    return (
        df.withColumn(
            "prev_is_operational", lag(col("is_operational"), offset=1).over(window)
        )
        .withColumn("rn", row_number().over(window))
        .withColumn("total_power", col("c.power_kw") * col("c.quantity"))
    )
...
def get_station_status_facts(df_joined):
    return df_joined.groupBy(col("s.station_id")).agg(
        ...
        max_by(col("is_operational"), col("s.date_last_status_update")).alias(
            "is_operational"
        ),
        spark_sum(
            when(
                ~col("is_operational")
                & (col("prev_is_operational") | col("prev_is_operational").isNull()),
                1,
            ).otherwise(0)
        ).alias("offline_count"),
        spark_sum(
            when(
                ...
                timestamp_diff(
                    "MINUTE",
                    col("s.valid_from"),
                    least(lit(datetime.now()), col("s.valid_to")),
                ),
            ).otherwise(0)
        ).alias("offline_duration"),
        timestamp_diff(
            "MINUTE", spark_max(col("s.date_created")), lit(datetime.now())
        ...
        .alias("risk_severity_level"),
        spark_sum(
            when(col("rn") == 1, round(0.35 * col("total_power"))).otherwise(0)
        ).alias("avg_energy_cost_per_hour"),
    )
...
def add_station_facts_metadata(df):
    return (
        ...
        .withColumn(
            "availability",
            round((1 - (col("offline_duration") / col("total_duration"))), 2),
        )
...
def get_station_dim(df_joined):
    ...
    return (
        df_joined.groupBy(col("s.station_id"))
        .agg(*[spark_max(col(v)).alias(k) for k, v in cols.items()])
        .withColumn("ingest_timestamp", lit(datetime.now()))
    )
...
def get_weather_dim(df_joined):
    ...
    return (
        df_joined.groupBy(col("w.weather_sk"))
        .agg(*[spark_max(col(v)).alias(k) for k, v in cols.items()])
        .withColumn("ingest_timestamp", lit(datetime.now()))
    )
