...
def test_join_tables_operational_flag(spark):
    ...
    operational_status = result_df.filter(col("s.station_id") == 1) \
        .select("is_operational").collect()[0][0]
    assert operational_status is True

    # Station with status_id 30 should not be operational
    non_operational_status = result_df.filter(col("s.station_id") == 2) \
        .select("is_operational").collect()[0][0]
...
def test_add_station_facts_metadata(spark):
    ...
    df = spark.createDataFrame(
        [(1, 100.0, 60.0, 1000.0)],  # 60 min offline out of 1000 min total
        schema
    )
...
def test_get_station_dim(spark):
    ...
    df_stations = spark.createDataFrame([
        (1, 50, 40.7, -74.0, "Station A", "123 Main St", "New York", "NY", 40.7, -74.0),
        (2, 50, 40.8, -73.9, "Station B", "456 Oak Ave", "Newark", "NJ", 40.8, -73.9)
    ], stations_schema)
    ...
    df_connections = spark.createDataFrame([
        (1, 50.0),
        (1, 100.0),  # Same station, different connector
        (2, 75.0)
    ], connections_schema)
...
def test_get_weather_dim(spark):
    ...
    df_weather = spark.createDataFrame([
        (40.7, -74.0, "Clear", 1, 100, "clear sky", 72.0, 1013, 65, 10000, 0.0, 0.0),
        (40.7, -74.0, "Clear", 1, 100, "clear sky",
         72.0, 1013, 65, 10000, 0.0, 0.0),  # Duplicate
        (40.8, -73.9, "Cloudy", 2, 101, "broken clouds",
         68.0, 1010, 70, 8000, 0.0, 0.0)
    ], weather_schema)
...
def test_risk_score_calculation(spark):
    ...
    df_weather = spark.createDataFrame(
        [(40.7, -74.0, "Thunderstorm", 1, 100, datetime(2025, 1, 1),
          "thunder storm", 100.0)],
        weather_schema)
